...
try {
  ...
  console.warn('Could not initialize @google/generative-ai — running in demo fallback mode.', err && err.message ? err.message : err);
}
...
async function generateNextQuestion(conversationHistory, userAnswer, interviewType) {
  try {
    ...
    if (!geminiAvailable || !model) {
      ...
      const nextQ = conversationHistory.length < 1
